...
class KukaGymEnv(gym.Env):
  ...
  def _reset(self):
    ...
    self.blockUid =p.loadURDF(os.path.join(self._urdfRoot,self._objectName), xpos,ypos,-0.15,orn[0],orn[1],orn[2],orn[3])
  ...
  def get_rgb(self, W=960, H=720, dist=None, pitch=None,
                yaw=None, roll=None):
    if dist is None:
      dist = self._cam_dist
    if pitch is None:
    ...
    if roll is None:
      roll=0
    base_pos,orn = self._p.getBasePositionAndOrientation(self._kuka.kukaUid)
    base_pos = (.45, base_pos[1], base_pos[2])
    view_matrix = self._p.computeViewMatrixFromYawPitchRoll(
        cameraTargetPosition=base_pos,
        distance=dist,
        yaw=yaw,
  ...
  def get_depth(self, W=960, H=720, dist=None, pitch=None,
                yaw=None, roll=None):
    if dist is None:
      dist = self._cam_dist
    if pitch is None:
    ...
    proj_matrix = self._p.computeProjectionMatrixFOV(
        fov=60, aspect=float(W)/H,
        nearVal=0.1, farVal=100.0)
    (_, _, _, depth, _) = self._p.getCameraImage(
        width=W, height=H, viewMatrix=view_matrix,
        projectionMatrix=proj_matrix, renderer=self._p.ER_BULLET_HARDWARE_OPENGL)

    return depth

  def perform_grasp(self, env, steps=300):
    fingerAngle = 0.3
    for i in range (100):
      graspAction = [0,0,0.0001,0,fingerAngle]
